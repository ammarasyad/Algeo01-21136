...
public class ImageUpscale {
    public static BufferedImage scale(BufferedImage input) {
        ...
        for (int y : new int[]{0, height - 2}) {
            for (int x = 0; x <= width - 2; x += 2) {
                ...
                for (int i = 0; i < 4; i++) {
                    System.arraycopy(result[i], 0, newPixels[y * 2 + i], x * 2, 4);
                }
            }
        }
        ...
        if (width % 2 == 1) {
            for (int y = 0; y < (height / 2) * 2 - 1; y += 2) {
                ...
                int[][] result = interpolateEdgePixels(subPixels, 2);

                for (int i = 0; i < 4; i++) {
                    System.arraycopy(result[i], 0, newPixels[y * 2 + i], (width - 1) * 2, 2);
                }
            }
        }
        ...
        if (height % 2 == 1) {
            for (int x = 0; x < width - 2; x += 2) {
                ...
                int[][] result = interpolateEdgePixels(subPixels, 1);

                for (int i = 0; i < 2; i++) {
                    System.arraycopy(result[i], 0, newPixels[(height - 1) * 2 + i], x * 2, 4);
                }
            }
        }
        ...
        if (height % 2 == 1 && width % 2 == 1) {
            ...
            int[][] result = interpolateEdgePixels(subPixels, 3);

            for (int i = 0; i < 2; i++) {
                System.arraycopy(result[i], 0, newPixels[(height - 1) * 2 + i], (width - 1) * 2, 2);
            }
        }
    ...
    public static int[][] interpolateEdgePixels(int[][][] subPixels, int mode) {
        int row, col;
    ...
    public static int[][] interpolateChannelBicubic(double[][] color) {
        ...
        int colorRow = color.length;
        DoubleMatrix mColor = new DoubleMatrix(colorRow);
        for(int i=0;i<colorRow;i++){
            int colorCol = color[i].length;
            for(int j=0;j<colorCol;j++){
                mColor.setElement(i, j, color[i][j]);
            }
        }
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                result[i][j] = (int) BicubicInterpolation.matrixInterpolation(i/3d, j/3d, mColor);
            }
        }
